...
func TestHeaders_ContainsRequiredFields(t *testing.T) {
	...
	nonCanonical := []string{"JWTToken", "loginType", "source-type"}
...
func TestHeaders_JWTTokenSet(t *testing.T) {
	...
	if len(vals) == 0 || vals[0] != "abc123token" {
		t.Errorf("JWTToken header = %v, want %q", vals, "abc123token")
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c, cleanup := testServerClient(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				if pk := r.Header.Get("JWTToken"); pk != "test-key" {
					t.Errorf("JWTToken = %q, want %q", pk, "test-key")
				}
...
func TestRequestURL_GatewaySigned(t *testing.T) {
	...
	mac := hmac.New(sha256.New, []byte(color))
