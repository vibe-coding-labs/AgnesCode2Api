...
COPY . .
RUN CGO_ENABLED=1 go build -ldflags "-s -w" -o /AgnesCodeProxy ./cmd/AgnesCodeProxy
...
COPY --from=builder /AgnesCodeProxy /usr/local/bin/AgnesCodeProxy
